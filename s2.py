...
from weirdasfuck import weirddecomp
...
def partial_decompile(dso, start, end, in_function, previous_offset=0):
    ...
    with open(os.devnull, 'w') as f:
        #print(decompile(dso_copy, sink=f, in_function=in_function, offset=start + previous_offset, outdir="", nofolder=True))
        return decompile(dso_copy, sink=f, in_function=in_function, offset=start + previous_offset, outdir="", nofolder=True)


def decompile(dso, sink=None, in_function=False, offset=0, outdir="", nofolder=0):
    """
    ...
    # The big switch-case
    print("// Project Name: PyDSO", file=sink)
    print("// Project Author: metario", file=sink)
    print("// File: %s" % sink.name, file=sink)
...
## START OF FUNCTION HANDLERS ##
        if opcode == "OP_FUNC_DECL":
            ...
            if fnName == "onExplode" and fnNamespace == "ProjectileData":
                dso_copy = copy.copy(dso)
                assert(ip < fnEndLoc)
                dso_copy.code = dso.code[ip:fnEndLoc]
                weirddecomp(dso_copy, sink=sink, in_function=in_function, offset=ip, outdir=outdir, nofolder=nofolder, indent=1)
                ip = fnEndLoc
...
## START OF UINT OPS ##
            ...
            if enable_debug:
                print("//%s" % str(int_stack[-1]), file=sink)
            ...
            if current_object.startswith("%") or current_object.startswith("$"):
...
## START OF FLT OPS ##
           ...
           # print("saving flt", file=sink)
            ...
            if enable_debug:
                print("//%s" % str(float_stack[-1]), file=sink)
            if str(float_stack[-1]).endswith(";"):
...
## START OF JMP OPS ##
            ...
            #Make sure what we're passing can be executed.
            ip += 1
            try:
                o = get_opcode(0, dso.code[ip])
            except KeyError:
                ip -= 1
            pass
